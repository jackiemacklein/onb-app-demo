...
class Content extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (window.location.href.includes("/relatorios/gerenciais/caixas")) {
  ...
  render() {
    let { cashiers, page_length, page, page_limit } = this.state;
    let pages = [];

    // --> obtem os registros filtrados, conforme a busca pelo usuário
    let items = cashiers.filter(item => this.state.term === "" || this.filterHistory(item));
                  ...
                  <Card key={item.id} className="card">
                    <CardBody>
                      <Link to={`/caixa/${item.id}`} target="_blank">
                        <CardTitle className="h2">Aberto às {format(parseISO(item.date), "HH':'mm'Hrs'")}</CardTitle>
                      </Link>
