...
class Content extends Component {
  ...
  async save(event) {
    ...
    try {
      await api.put(`/users/${this.state.id}`, {
  ...
  async loadData() {
    ...
    this.setState({
      ...
      name: data.name,
      email: data.email,
      method_login: { value: data.method_login, label: this.state.methods_login.find(m => m.value == data.method_login).label },
      password: data.password,
      profile_id: { value: data.profile.id, label: data.profile.name },
      filial_id: { value: data.filial.id, label: data.filial.name },
      is_active: data.is_active,
      sales_commission: data.sales_commission,
      sales_cancel: data.sales_cancel,
      selected_modules: await data.modules_aux.map(d => {
        return { value: d.id, label: d.title };
      }),
      selected_spaces: await data.spaces_aux.map(d => {
        return { value: d.id, label: d.name };
      }),
    });
  }
  ...
  render() {
    ...
    return (
      <Fragment>
        <h2 id="formsBase">Dados do Usuário: {this.state.name}</h2>
        <form className="rui-snippet-preview demo form-border-effect-kie" onSubmit={this.save}>
          ...
          <FormGroup>
            <Label for="email">{this.state.method_login.label} *</Label>
            <Input
              type="text"
              name="email"
              required
              id="email"
              placeholder={this.state.method_login.label}
          ...
          <FormGroup>
            ...
            <Select
              ...
              value={this.state.selected_modules}
              options={modules}
              styles={customStyles}
              isMulti
          ...
          <FormGroup>
            ...
            <Select
              ...
              options={spaces}
              styles={customStyles}
              value={this.state.selected_spaces}
              isMulti
