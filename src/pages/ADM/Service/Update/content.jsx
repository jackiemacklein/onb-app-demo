...
class Content extends Component {
  ...
  async save(event) {
    ...
    try {
      await api.put(`/services/${this.state.id}`, { ...this.state });
  ...
  async loadData() {
    ...
    this.loadSpaces(data.values);
  }

  async loadSpaces(param) {
    const { data } = await api.get("/spaces");
    this.setState({
      values: data.map(item => {
        let value = param.find(v => v.space_id === item.id);
        if (value) value = value.price;
        else value = "0,00";

        return { space_id: item.id, name: item.name, price: value };
      }),
    });
  }
